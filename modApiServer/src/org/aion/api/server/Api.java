...
public abstract class Api<B extends AbstractBlock<?, ?>> {
    ...
    protected Map<String, CompiledContr> contract_compileSolidityZip(final byte[] zipfile,
        String entrypoint) {
        try {
            Compiler.Result res =
                solc.compileZip(zipfile, entrypoint, Compiler.Options.ABI, Compiler.Options.BIN);
